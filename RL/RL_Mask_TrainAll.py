...
# Make sure this import points to your maskable, gymnasium-based environment!
from RL.RL_Env_Maskable import DataSelectionEnvMaskable
...
def train_model(T, UR, sources, alpha, beta, gamma, save_path):
    ...
    env = DataSelectionEnvMaskable(sources, UR, source_stats, value_index, alpha, beta, gamma)
    model = MaskableDQN(
        "MlpPolicy", env,
        verbose=0,
        buffer_size=10000,
...
def run_all():
    ...
    ur_cases = [20]  # Add more if needed
    source_variants = {
        ...
        for variant_name, variant_fn in source_variants.items():
            ...
            for alpha in alpha_values:
                for beta in beta_values:
                    ...
                    if gamma < 0:
                        continue
                    save_dir = f"results_masking/case_{case_id}/{variant_name}/alpha_{alpha}_beta_{beta}"
    ...
    np.savez("all_training_metrics_masking.npz", **all_metrics)
    print("All training metrics saved to all_training_metrics_exploration.npz")
